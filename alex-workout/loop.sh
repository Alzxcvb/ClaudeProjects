...
# Scoped tool permissions — alex-workout is plain HTML/JS/CSS, no build, no DB.
ALLOWED_TOOLS="Write,Read,Edit,MultiEdit,Bash(git add *),Bash(git commit *),Bash(git diff *),Bash(git log *),Bash(git status),Bash(git restore *),Bash(node --check *),Bash(node -e *),Bash(grep *),Bash(tail *),Bash(head *),Bash(ls *),Bash(cat *),Bash(wc *)"
...
log "Starting Ralph Loop: Alex Workout"
