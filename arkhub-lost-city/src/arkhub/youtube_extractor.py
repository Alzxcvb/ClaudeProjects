...
def list_channel_videos(config: YouTubeConfig) -> list[dict[str, Any]]:
    ...
    try:
        for video in scrapetube.get_channel(config.channel_handle):
            videos.append({
...
def fetch_transcript(video_id: str, language: str = "en") -> str | None:
    ...
    try:
        ...
        # Silently return None for videos without transcripts
        return None
...
def run_extraction(config: YouTubeConfig, root: Path) -> dict[str, Path]:
    ...
    for i, video in enumerate(videos):
        ...
        transcript_text = fetch_transcript(video_id, config.language)
